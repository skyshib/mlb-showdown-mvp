...
2. Choose 1–5 starting pitchers per team, then draft the resulting roster:
   ...
   - 2 bullpen pitchers
   - Optionally, **coaches**: tick `Deal the coaching staff` on the setup screen and six of the twelve coaches, drawn by the seed, join the board in place of the spare DH bats. A coach takes no roster slot and no team has to draft one, but each costs a pick (or a bid) and does one small thing to how your club plays — a first base coach who gives Speed A runners +1 on steals, an outfield coach worth +1 on every outfield throw, a lefty specialist, a late-innings hitting coach, a catching coach who frames two-out ties, a platoon coach, a contact coach who trades strikeouts for ground balls, a rotation coach who gets the ace three starts in five, a bullpen coach who turns a fly-ball face into a strikeout, a clutch gene that carries a natural 20 past the die, an old-school infield coach whose club never runs, and the Wild Card, who flips a coin on one hitter you name: +1 or -1 on every swing, decided the moment you name him. See [docs/rules.md](docs/rules.md#coaches-optional-draft-mode).
...
- THE GAUNTLET: a mode where every card in the league is yours on day one — no shops, packs, coins, or card claims — and the whole game is your point cap. Build one club, lock it on the first pitch, and face six elite opponents (two gyms, four bosses) back to back in best-of-threes; lose a series and the run is over, recorded at the depth you reached. Three tiers set how far above your cap the six build: CONTENDER (1.1x), ELITE (1.4x, the default), and IMMORTAL (the World Series budget, better than two points to your one).
...
- Dead money: in an auction, what each manager paid for the cards not in the lineup or on the staff, on the roster panel, the recap, the rival-boards dock, and the TV board. It reads off the bench as it stands, so it moves whenever a lineup does.
- Coaches, an optional draft mode: six of twelve clipboard cards, drawn by the seed, dealt into the board in place of the spare DH bats, each a small tweak to the rules for the club that drafts it. Any number per team, none required; a coach costs a pick but no slot, so a snake draft simply runs a turn longer for whoever spent one.
