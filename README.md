...
- Coaches, an optional draft mode: ten of twelve clipboard cards, drawn by the seed, dealt on top of the board, each a small tweak to the rules for the club that drafts it. Any number per team, none required; a coach costs a pick but no slot, so a snake draft simply runs a turn longer for whoever spent one.
