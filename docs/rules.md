...
- `Sim N games` runs a balanced stream of seeded games, cycling through every team pairing with seeds `{roomSeed}-batch-game-{n}-{away}-{home}`, so the same room seed always reproduces the same batch. The batch screen reports win percentage, 162-game pace stats, per-player aggregate lines, and sim awards.
...
- Every plate appearance and steal attempt carries a win-probability estimate (`wpBefore`/`wpAfter` on the event log). The model projects the final run differential as a normal distribution: current lead, plus a scaled MLB run-expectancy table for the half in progress, plus the expected runs for each remaining half-inning. The run environment (mean and variance of runs per half-inning) is not hardcoded: each batch plays a short seeded calibration slate between the drafted teams and measures it, because the pools differ wildly (real ≈0.25 runs/half, Mariners ≈0.17, generated ≈0.70) and any fixed constant goes stale when cards get rebalanced. Games simulated without a calibration pass fall back to a pooled default. Ties resolve to a coin flip through a continuity band. It is an approximation for storytelling, not a claim about official Showdown odds.
